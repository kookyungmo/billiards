...
urlpatterns += patterns('billiards.views.assistant',
    ...
    url(r'^assistant/(?P<assistant_uuid>%s)/order/(?P<transaction_id>\d{1,})/reject' %(UUID_PATTERN), 'assistant_order_reject_by_tid', name='assistant_order_reject'),
    url(r'^assistant/(?P<assistant_uuid>%s)/order/(?P<transaction_id>\d{1,})/accept' %(UUID_PATTERN), 'assistant_order_accept_by_tid', name='assistant_order_accept'),
