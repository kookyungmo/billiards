...
def response_msg(request):
        ...
        if len(nearbyPoolrooms) > 0:
            ...
            if coupons.count() > 0:
                echopictext = picTextTpl % (
                                 ...
                                 (picTextItemTpl %(title, discription, picurl, originContent) + getCouponsText(coupons))) 
            else:
                echopictext = picTextTpl % (
                         msg['FromUserName'], msg['ToUserName'], str(int(time.time())), 1,
                         (picTextItemTpl %(title, discription, picurl, originContent)))
            return echopictext
        else:
    ...
    #response image message
        ...
        echopictext = picTextTpl % (
                            msg['FromUserName'], msg['ToUserName'], str(int(time.time())), 1,
                            (picTextItemTpl %('hello', 'hi', pic_url, '')))
        echopic = picRpl % (
                            msg['FromUserName'], msg['ToUserName'], str(int(time.time())),
                            pics[picid])
        return echopic
    ...
    #response text message
        ...
        def getCouponText(coupons):
            ...
            echopictext = picTextTpl % (
                                        ...
                                        getCouponsText((coupons)) + (picTextItemTpl %("俱乐部详情", coupon.poolroom.name, picurl, weblink)))      
            return echopictext
        ...
        if msg['Content'] in content.keys():
            ...
            echopictext = picTextTpl % (
                             msg['FromUserName'], msg['ToUserName'], str(int(time.time())), 1,
                             (picTextItemTpl %(videos[videoid]['title'], videos[videoid]['description'], videos[videoid]['plink'], videos[videoid]['vlink'])))         
            return echopictext
            ...
            if acts.count() > 0:
                def getActsText(acts):
                    ...
                    for act in acts:
                        ...
                        text.append(picTextItemTpl %(act.title, act.starttime, picurl, request.build_absolute_uri(reverse('activity_detail', args=(act.pk,)))))
                    return ''.join(text)
                echopictext = picTextTpl % (
            ...
            if matches.count() > 0:
                def getMatchesText(matches):
                    ...
                    for match in matches:
                        ...
                        text.append(picTextItemTpl %(match.title, match.starttime, picurl, request.build_absolute_uri(reverse('match_detail', args=(match.pk,)))))
                    return ''.join(text)
                echopictext = picTextTpl % (
...
def getCouponsText(coupons):
    ...
    for coupon in coupons:
        picurl = buildPoolroomImageURL(coupon.poolroom)
        text.append(picTextItemTpl %(coupon.title, coupon.description, picurl, coupon.url))
    return ''.join(text)
