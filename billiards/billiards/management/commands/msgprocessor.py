...
class Command(NoArgsCommand):
    ...
    def call(self, function_string, *args):
        import importlib
    ...
    def hsSendMsg(self, number, content):
    ...
    def handle(self, *args, **options):
        ...
        bcmsmsg = BcmsMessage.objects.first()
        try:
            while True:
                    ...
                    if 'error_code' in msgs:
                        ...
                        for msg in msgs['response_params']['messages']:
                            ...
                            try:
                                ...
                                cellphone = data['number']
                                logger.info('[%s]Sending message \'%s\' to number %s' %(localtime(now()).strftime(TIME_FORMAT), decodeunicode(data['msg']), cellphone))
                                data = simplejson.loads(data['msg'])
                                messagecontent = self.call(data['method'], data)
                                logger.debug(messagecontent)
                                self.hsSendMsg(cellphone, messagecontent)
                            except ValueError, e:
